...
1) Load the data from a package assicated with our textbook.
...
Q: What are the names and types of variables included in this dataset?
...
To make a scatterplot (X, Y):
plot(???_variable, ???_variable)
...
prop.table(table(???_variable))



4) Make an appropriate graph and/or table for age and another for age.cohort and describe their distributions.  Do consumers tend to be older or younger?
...
9) Do annual expenditures seem to vary by gender?
...
11) The following code will let you examine the data for the two largest ethnic groups, Hispanic and White non-Hispanic, which comprise the majority of the data.  In prior exercises, we used the subset() function to examine portions of the data independently. Here, we will subset data using brackets that specify specific criteria of interest. What do each of the components of this line of code accomplish?  Do Hispanic consumers, on average, seem to sreceive less financial support from the California DDS than a White non-Hispanic consumer?
...
Lastly, calculate the difference between Hispanic and White non Hispanic average expenditures across age cohorts.  Based on this exploratory analysis, does there seem to be evidence of ethnic descrimination in the amount of financial support provided by the California DDS? Summarize your findings in language accessible to a non-statistician.
